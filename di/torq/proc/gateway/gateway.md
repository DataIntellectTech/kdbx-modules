...
are the two vendored modules, so di.torq.proc.gateway itself is small.
...
so di.torq.proc.gateway passes the injected `log` straight through (no adapter, unlike di.dbwrite).
...
`(`.gw.reload;`reloadend)` after the hdb/rdb have reloaded (di.torq.proc.wdb's `informgateway` already sends
...
- **Attribute routing needs backend attribute reporting.** di.serverselect can route by an attribute
  ...
  di.torq.servers/di.torq.proc.rdb/di.torq.proc.hdb don't yet — so v1 registers backends with **empty attributes** and
...
q)k4unit.moduletest`di.torq.proc.gateway
```
