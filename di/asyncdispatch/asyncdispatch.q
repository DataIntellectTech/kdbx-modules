...
addquery:{[query;servertype;join;postback;timeout;sync]
  ...
  .z.M.queryqueue upsert (queryid;.z.m.cp[];.z.w;query;servertype;join;{$[11h=type x;enlist x;x]}postback;timeout;0Np;0b;sync);
...
init:{[deps]
  ...
  / housekeeping (checktimeout; removequeries; removeinactive) is the caller's responsibility -
  / wire them into your timer after init; the exported defaults for age params are querykeeptime
  / and clearinactivetime
  / examples:
