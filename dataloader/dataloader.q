...
.loader.loaddata:{[loadparams;rawdata]
  ...
  data:$[loadparams[`filename] in .loader.filesread; / check if some data has already been read in
...
.loader.writedatapartition:{[dbdir;partition;partitiontype;partitioncol;tablename;data]
  ...
  .loader.partitions[writepath]:(tablename;partition);
  };
...
.loader.finish:{[loadparams]
  ...
  {.loader.util.sorttab (x;where .loader.partitions[;0]=x)} each distinct value .loader.partitions[;0];
  system"x .z.zd";
  if[loadparams`gc; .Q.gc[]];
  };
...
.loader.loadallfiles:{[loadparams:.loader.util.paramfilter;dir]
  ...
  {[loadparams;file] .Q.fsn[.loader.loaddata[loadparams,(enlist`filename)!enlist file];file;loadparams`chunksize]}[loadparams] each filelist;
  .loader.finish[loadparams];
  };
...
.loader.util.sorttab:{[d]
  ...
  {[sp;dloc] / loop through each directory and sort the data
    ...
    if[count attrcols: select column, att from sp where not null att;
      .loader.util.applyattr[dloc;;]'[attrcols`column;attrcols`att]]; / apply attributes
...
.loader.util.paramfilter:{[loadparams]
  ...
  if[not 99h=type loadparams; '".loader.loadallfiles requires a dictionary parameter"];
...
.loader.init:{[sortparams:.loader.util.sortfilter]
  ...
  .loader.partitions:()!(); / maintain a dictionary of the db partitions written to by loader
