...
modvendorns:{[modname]
  ...
  / kx.log), so dependency resolution (getexport/checkonedep/checkonecontract) must not hardcode `.m.di - only
...
checkmoduledeps:{[modshort]
  / checks one already-loaded module's declared deps.q (if any) against the current session
  / a malformed deps.q (present but not a dict) is reported as its own failure rather than left to throw a raw
  / q type error out of checkonedep'[key d;value d] - a crash there would abort the whole checkdeps[] walk (each
  / does not isolate per-element errors), masking every other module's real failures behind one bad file
  modname:shorttofull modshort;
  d:readdepsq modname;
  $[d~(::);();
    not 99h=type d;enlist string[modname]," deps.q is malformed - expected a dict, got type ",string type d;
    raze checkonedep'[key d;value d]]
  };
...
checkonecontract:{[provider;required]
  / if provider is loaded, checks its export dict contains every key its known contract requires
  / vendor-agnostic like checkonedep/getexport, though every current contracts entry happens to be di.*-prefixed
  ...
  missing:required where not required in key xp;
...
checkcontracts:{[]
  ...
  raze checkonecontract'[key contracts;value contracts]
  };
