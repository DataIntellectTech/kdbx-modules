...
- **Core dependency contracts** — for whichever of `di.util.log`, `di.timer`, or `di.torq.handlers` are loaded, the export dict is checked for the required keys of the contract it provides (Logging: `info`/`warn`/`error`; Timer: `addjob`/`deletejobs`/`enablejobs`/`disablejobs`/`getactivejobs`/`cp`; Handlers: `register`/`remove`/`list`). The single-contract check is also exported directly as `checkcontract[provider;requiredkeys]` for a contract this module doesn't know by name.
...
The `log` dependency must be passed to `init` inside the `deps` dict keyed on `` `log ``, and must already match the binary `{[c;m]}` contract — the module validates key presence but does not detect or adapt other shapes (e.g. a monadic `kx.log` instance). To use `di.util.log`, pass its `logdict`.
...
- **A version may be published in either place; both audits accept both.** The pre-load `checkversiongraph` reads a module's on-disk `VERSION` file (it must not load code); the post-load audit reads the loaded module's `version` **export**. These are the same source of truth published two ways, so `checkdepversion` falls back to the `VERSION` file when a loaded module withholds `version` from its export dict. Several modules legitimately do — `di.torq.servers`, `di.torq.config`, `di.util.toml` and di.torq.depcheck itself keep it out of `export` because their `getapimeta` contract test asserts the exact export set. Without the fallback, every process whose manifest declares such a module fails this audit *after* a completely successful init: `di.torq.proc.rdb`, `di.torq.proc.wdb` and `di.torq.proc.gateway` all declare `di.torq.servers`, and all three died exactly there — a failure mode only a real stack boot surfaced, since those modules' unit suites do not boot a process. A module carrying neither an export nor a file is still reported as before.
