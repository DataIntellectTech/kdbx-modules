...
/ di.torq.proc.chainedtp wired the way di.torq wires it: the REAL di.util.log, di.timer, di.torq.handlers and - built
/ last, as it consumes the other three - di.torq.servers, whose init takes this process's identity and the process.csv
/ phone book from the config. Every -flag value on the command line becomes a config setting as a STRING (as a .toml
/ or a command-line override would deliver it), so the module's own coercions are exercised: -upstreamtype -tplogdir
/ -publishmode -pubperiod -replay -clearlogonsubscription -subscribeto -subscribesyms -connecttimeoutms -processcsv
/ -proctype -procname
