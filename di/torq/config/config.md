...
`overrideconfig`). `di.torq.config` resolves a settings **cascade** over a builtin root and an
...
`di.torq.config` resolves config into a **returned flat dict**; it does not execute settings
files into namespaces or hold any global store. This is the shape `di.torq` consumes:

```q
config:use`di.torq.config
...
> **Note — name-major, not root-major.** The KDBX-POC's own vendored `di.torq.config` used
> *root-major* precedence (all builtin tiers, then all app tiers), where `app/default`
> would beat `builtin/{proctype}`. `di.torq.config` deliberately keeps the **name-major** rule
...
- **`.toml`** — delegated to **`di.util.toml`**, loaded lazily (only when a `.toml` file is
...
```q
/ option 1: di.util.log — the standard logger (exports binary info/warn/error)
...
first — di.torq.config does no wrapping.
...
|---|---|---|
| `log` | `` `info`warn`error `` | `{[c;m]}` (context symbol, message string). Caller passes an already-conforming binary dict — built from `di.util.log` or hand-rolled. No adaptation is done in the module. Needed only by `overrideconfig`. |

## Hard dependencies

None mandatory. `cascade`/`parsefile` gain a **lazy, soft dependency on `di.util.toml`**: it is
`` use``d only when a `.toml` file is actually encountered, so the `.q`-only path needs
nothing. **di.util.toml does not need to be in the repo (or on `QPATH`) for di.torq.config to load or
run** — it is required only at the moment a `.toml` settings file is actually parsed.

### The `.toml` guard rail

`parsefile` guards the di.util.toml load so a missing module produces a clear, actionable error
...
1. **Check the file exists first.** A missing file (either extension) → empty dict. So a
   missing `.toml` tier *never* triggers the di.util.toml requirement — the internal
   `parsetier`/`cascade` probe `base,".toml"` on every tier, and absent tiers cost nothing.
2. **Then, only for a `.toml` file that actually exists, require di.util.toml.** The internal
   `requiretoml` helper attempts `` use`di.util.toml `` under protected evaluation; if it is not
   found, it signals a clear error **naming the offending file**:
   ```
   'di.torq.config: cannot parse TOML file '/path/to/x.toml' - the di.util.toml module was not found
    on QPATH; a di.util.toml module is required to parse .toml settings (underlying: notfound: di.util.toml)
   ```
3. **Parse.** With di.util.toml resolved, the file is delegated to it.
...
### Contract di.torq.config expects from di.util.toml

A di.util.toml module must export a **`parsefile`** function taking a settings-file path string
and returning a **flat dict** of `setting → value`:

```q
(use`di.util.toml)[`parsefile] "/path/to/settings.toml"   / -> `dir`rows!(":appdb";100)
```

TOML has no symbol type, so di.util.toml returns every TOML string as a **q char string** (never
a symbol); di.torq.config applies no coercion, so consumers that need a symbol normalise at the
point of use (`` `$ `` is a no-op on an already-a-symbol value).

> **⚠️ `di.util.toml` is not yet in kdbx-modules** — it currently lives only in the KDBX-POC and
> lands here with the PoC merge (a di.util.toml module is in development against the contract
...
> works standalone. Full `.toml` resolution is verified in the PoC (where `di.util.toml` is
> vendored); this repo's suite covers the guard's error path (di.util.toml absent) and the
...
- **`overrideconfig` is the command-line layer, applied by `di.torq` — not a manual step.**
  ...
  `.Q.opt .z.x` so launch-time flags beat the settings files. di.torq.config just does the
...
- **String-typed (`10h`) settings are overridable — as text, no parse.** A char-string
  ...
  could. di.torq.config stays policy-free: it preserves whatever type the setting already had
...
k4unit.moduletest`di.torq.config
```

The `.toml` path is exercised in the KDBX-POC (where `di.util.toml` is vendored), not here —
see the hard-dependencies note above.

## Follow-ups (deferred until other modules land)

di.torq.config is complete for v1 in isolation. The items below are intentionally deferred;
each is keyed to the module or milestone that unblocks it.

### ⚠️ Logging call convention — NOT set in stone (ask before changing)

di.torq.config uses the **three-flat-var** convention (`.z.m.loginfo`/`.z.m.logwarn`/`.z.m.logerr`),
matching `consistency.md` and `di.compression`, but the project hasn't finalised this versus
the **single-dict** form (`.z.m.log[\`info][…]`). **Before changing di.torq.config's logging — or
...
### When `di.util.toml` lands in kdbx-modules (with the PoC merge)

- **Add committed `.toml` tests.** Today `di.util.toml` is not on this repo's `QPATH`, so a
  committed `.toml`/mixed-tier test would fail here (verified in the PoC instead). Once
  `di.util.toml` resolves, add tests covering: a `.toml` tier parsed into the dict, `.toml`
  winning over `.q` within a tier, and a mixed `.q`+`.toml` cascade.

### When `di.util.log` merges to main

- **Add a committed `di.util.log` integration test.** The contract match (binary
  `` `info`warn`error `` dict) is verified manually and stood in for by the kx.log-wrapper
  emission test. Once `di.util.log` resolves on `QPATH`, add a test that builds the dict from
  `di.util.log` and drives `overrideconfig` through it.
...
- **Hold the env-free boundary.** di.torq.config reads no env vars or process identity — di.torq
  owns that resolution and passes the roots and identity explicitly.

### When `di.torq.depcheck` is built (versioning rollout)

- **Add a `version` export and `deps.q`.** When `di.torq.depcheck` lands, add `version:"…"` to the
  export and a minimal `deps.q` (di.torq.config has no hard deps) as part of the coordinated
  repo-wide rollout, not a di.torq.config-only change.
