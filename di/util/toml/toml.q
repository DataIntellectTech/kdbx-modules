...
/ pure: no init/logger/deps - di.torq.config loads it during config resolution, before a logger exists.
/ parsefile signals on a MISSING file; a caller for whom that is acceptable (a cascade probing
/ optional tiers) guards existence itself first, as di.torq.config does: if[0=count key hsym`$path;:()!()].
