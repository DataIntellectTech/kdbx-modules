...
readdepstoml:{[modname]
  ...
  / (file-existence-gated, exactly like di.torq.config's parsefile checks existence before dispatching on extension). a missing
  / or broken di.util.toml degrades to one aggregated failure line worded like di.torq.config's requiretoml (name the file/module,
  / name the underlying cause, one sentence) - but deliberately does NOT throw: di.torq.config's requiretoml aborts its whole
...
readdeps:{[modname]
  ...
  / clash - mirroring di.torq.config's live parsetier ((parsefile base,".q"),parsefile base,".toml"). returns (failures;dict):
...
checkdepversion:{[dep;minver]
  ...
  ver:$[xp~(::);"";$[`version in key xp;xp`version;""]];
  / FALL BACK to the on-disk VERSION file when a loaded module withholds `version` from its export dict. Several
  / modules legitimately do: di.torq.servers/di.torq.config/di.util.toml/di.torq.depcheck all ship a VERSION file
  ...
  if[0=count ver;
    ...
    if[not dir~(::);ver:readversion dir]];
  / neither source carries a version - the original message still reads correctly (di.timer/di.tz ship neither)
  if[0=count ver;:noversionmsg];
  checkfoundversion[dep;minver;ver]
  };
...
checkdeps:{[]
  ...
  / fixtures on disk (di.torq.handlers required at both a satisfied and an unsatisfied minimum simultaneously) since
...
checkcontracts:{[]
  / checks whichever of the known core-dependency providers (di.util.log/di.timer/di.torq.handlers) are loaded in this session
...
ztscheck:{[]
  ...
  / di.torq.handlers, which explicitly excludes .z.ts from its own scope), so there is no ownership marker to check
  / instead. Scope is deliberately limited to .z.ts only - whether this should ever cover other .z.* events is an
  / open question for di.torq.handlers' owner, not decided here
  bound:not (::)~@[get;`.z.ts;{(::)}];
  if[not bound;:()];
...
init:{[deps]
  ...
  if[not `log in key deps;'"di.torq.depcheck: log dependency is required; pass `info`warn`error functions - see di.util.log"];
