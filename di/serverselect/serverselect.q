...
init:{[deps]
  ...
  / holding a binary `info`warn`error dict of {[c;m]} loggers (from di.util.log or hand-rolled).
...
addserverfull:{[h;pname;st;hp;att]
  ...
  if[not -6h=type h;.z.m.logerr[`addserverfull;err:"di.serverselect: handle must be an int; got type ",string type h];'err];
  if[not -11h=type st;.z.m.logerr[`addserverfull;err:"di.serverselect: servertype must be a symbol; got type ",string type st];'err];
...
setserveractive:{[h;a]
  ...
  if[not -6h=type h;.z.m.logerr[`setserveractive;err:"di.serverselect: handle must be an int; got type ",string type h];'err];
  if[not -1h=type a;.z.m.logerr[`setserveractive;err:"di.serverselect: active flag must be a boolean; got type ",string type a];'err];
...
addserversfromtable:{[proctypes;conntable]
  ...
  if[not all `w`proctype`attributes in cols conntable;
    .z.m.logerr[`addserversfromtable;err:"di.serverselect: conntable must have columns w, proctype and attributes; got: ",", " sv string cols conntable];
    'err;
  ];
...
getservers:{[nameortype;lookups;req]
  ...
  if[(not `~lookups) and not nameortype in `servertype`procname;
    .z.m.logerr[`getservers;err:"di.serverselect: nameortype must be `servertype or `procname; got: ",string nameortype];
    'err;
  ];
  r:$[`~lookups;
...
selector:{[servertable;selection]
  ...
  if[not selection in `roundrobin`any`last;
    .z.m.logerr[`selector;err:"di.serverselect: unknown selection strategy: ",string selection];
    'err;
  ];
...
getserverids:{[att]
  ...
  if[count badkeys:reqkeys where 0h=type each att reqkeys;
    .z.m.logerr[`getserverids;err:"di.serverselect: attribute requirement values must be atoms or simple vectors; nested/mixed for: ",", " sv string badkeys];
    'err];
  ...
  if[all 0=count each serverids;
    .z.m.logerr[`getserverids;err:"di.serverselect: no servers match requested attributes"];
    'err;
  ];
  :serverids;
  };
...
getserveridsbytype:{[att]
  ...
  if[not 11h=abs type att;
    .z.m.logerr[`getserveridsbytype;err:"di.serverselect: servertype must be a symbol list (11h) or attribute dict (99h)"];
    'err;
  ];
  ...
  if[any null att;
    .z.m.logerr[`getserveridsbytype;err:"di.serverselect: null servertype passed as argument",activeserversmsg];
    'err;
  ];
  if[count servertype except activeservers;
    .z.m.logerr[`getserveridsbytype;err:"di.serverselect: ",$[max not servertype in allservers;
...
getserveridstype:{[att;typ]
  ...
  if[all 0=count each serverids;
    .z.m.logerr[`getserveridstype;err:"di.serverselect: no servers match ",string[typ]," requested attributes"];
    'err;
  ];
  :serverids;
  };
...
getserversinitial:{[req;att]
  ...
  if[not count att;.z.m.logerr[`getserversinitial;err:"di.serverselect: no servers report all requested attributes"];'err];
...
getserverscross:{[req;att;besteffort]
  ...
  if[(count last util`remaining) and not besteffort;
    .z.m.logerr[`getserverscross;err:"di.serverselect: cannot satisfy query - cross product of all attributes cannot be matched"];
    'err;
  ];
...
getserversindependent:{[req;att;besteffort]
  ...
  if[(null alldone) and not besteffort;
    .z.m.logerr[`getserversindependent;err:"di.serverselect: cannot satisfy query - not all attributes can be matched"];
    'err;
  ];
