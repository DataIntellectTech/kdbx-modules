...
init:{[config;deps]
  if[not `log in key deps;'"di.torq.proc.hdb: log dependency is required - see di.util.log"];
  .z.m.log:deps`log;
  ...
  / publish the IPC-callable surface at a real root-level name - use`di.torq.proc.hdb loads this
