...
/ a conforming log dict (or one supplied by di.util.log)
...
/ a no-op log dep for illustration (di.util.log would supply a real one)
...
k4unit.moduletest`di.torq.handlers
```
...
```q
k4unit:use`di.k4unit
.m.di.0k4unit.KUltf .Q.dd[hsym`$.Q.m.mp`di.torq.handlers;`test_integration.csv]
.m.di.0k4unit.KUrt[]
...
- A `post` handler runs in the event's own execution context; under multithreaded input (a negative `\p` port) that is not the main thread, so a `post` that writes a global hits kdb's `'noupdate` restriction, as any `.z.pg` code would. di.torq.handlers' own dispatch only reads its registries and is unaffected.
