...
**`di.toml` is a soft, lazy dependency** — it is not declared in `deps.q` and not loaded at import time. It is resolved once (cached) and called **only** when a module being audited ships a `deps.toml` file. A process whose modules use only `deps.q` never loads it, so it is not required to be on QPATH in that case. If a `deps.toml` *does* exist and `di.toml` is missing or fails to parse it, that is reported as one aggregated failure line (see Notes) rather than throwing.
