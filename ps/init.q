...
.z.m.reqalldict:enlist[`]!();

/ subscription table with filters
.z.m.reqfilteredtbl:([]table:`symbol$();handle:`int$();filts:();columns:());
...
delhandle:{[t;h]
    ...
    if[t in key .z.m.reqalldict; @[`.m.ps.reqalldict;t;except;h]]
    if[not count .z.m.reqalldict[t];.m.ps.reqalldict _:t];
    };

/ remove handle from request-filtered-data table
delhandlef:{[t;h]delete from `.m.ps.reqfilteredtbl where table=t, handle=h};
...
subscribe:{[table;filters]
    ...
    if[`~table;:.z.s[.z.m.t;filters]];
    if[not`~table;
        :$[`~filters;suball[table];subfiltered[table;filters]]]
    };
...
init:{
    .z.m.t:$[.z.m.availtables |0=count .z.m.subtables;tables[]except`reqfilteredtbl;.z.m.subtables];
