...
remotely-triggerable reload. This is the plan's `di.torq.proc.hdb` process type, started by
...
Requires a `log` dependency (nothing else - `di.torq.proc.hdb` doesn't register timers or
...
`.z.m` inside the published `reload` function still correctly resolves to `di.torq.proc.hdb`'s
...
q)k4unit.moduletest`di.torq.proc.hdb
```
