...
in-memory TOML text) into q dicts. Its reason for existing is to back di.torq.config's `.toml`
resolution tier — di.torq.config lazily loads di.util.toml only when a `.toml` file is actually parsed.

**Pure module** — no `init`, no logger, no injected dependencies. di.torq.config invokes it *during
...
cascade probing optional tiers) must **guard existence itself** before calling — the way di.torq.config
...
- **Strings are policy-free.** TOML has no symbol type, so every TOML string parses to a q **char
  ...
  This is the contract di.torq.config and its consumers rely on.
...
| `parsefile` | `parsefile[path]` | Read and parse a `.toml` file into a dict. A **missing file signals** — a caller that treats missing as acceptable (a config cascade) guards existence itself first (as di.torq.config does). |
...
to the di.torq.depcheck rollout, as in di.torq.config/di.servers.

## di.torq.config integration contract

di.torq.config's `parsefile` delegates a `.toml` path via `` (use`di.util.toml)[`parsefile] path ``, expecting
a **flat dict** of `setting → value` back (section values are sub-dicts). This module satisfies that
directly. Once di.util.toml is on `QPATH` alongside di.torq.config, the `.toml` half of di.torq.config's cascade
(and `.toml > .q` within a tier) works, and di.torq.config's guard-rail error (`the di.util.toml module was
