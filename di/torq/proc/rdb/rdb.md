...
logger and di.torq.proc.rdb bridged it; that adapter was removed when dbwrite was re-synced from
...
module's private namespace; a bare `insert` under `-11!` replay does too). di.torq.proc.rdb therefore
...
q)k4unit.moduletest`di.torq.proc.rdb
```
